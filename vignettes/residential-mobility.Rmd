...
pacman::p_load("DiagrammeR", "ggplot2", "gganimate", "sf", "magrittr", "data.table")
pacman::p_load_current_gh('yihui/knitr')
...
Residential mobility or household relocation plays an important role in determining the urban traffic and the housing market activity. In this example, we create a simple residential mobility model which comprises of two stages. The first stage is for households to decide whether to move from their current residence. If they decide to move then they will go onto the second stage otherwise they will remain where they are. The second stage is for the moving households to decide where they will relocate to. 

```{r , echo = FALSE, fig.cap="Flowchart of the processes in the simple residential mobility model"}
mermaid("
graph LR
...
```{r }
# make the returned object into from `st_queen` a list.
class(nb_list) <- NULL 
...
Now we must create a transition event that is to move or not to move. Since this is a choice situation it is a classification problem. 

```{r}
TransitionMove <- R6::R6Class(
  classname = "TransitionMove", 
  inherit = TransitionClassification
  )
```

This transition class `TransitionMove` allows a probabilistic model such as the one we have just created, `move_rate`, for simulation (Monte Carlo).
...
event_ltmobility_residentialRelocation <- function(world) {
  ...
  TransMove <- TransitionMove$new(x = Hh, model = world$get("move_rate")$get())
  ...
  if (length(moving_household_ids) > 0) {
    ...
    # create place holders for choicesets and their probabilities
...
for (i in start_iter:end_iter) {
  ...
  count_ls[[i]] <-
    world$get("Household")$get_data()[, .(count = .N, iteration = i), zid]
}
...
```{r, eval = FALSE, include = FALSE, fig.align = "center", fig.asp = 1, fig.width = 7}
g <-
  ggplot(count) +
...
```{r gapgif, fig.show='animate', ffmpeg.format='gif', dev='jpeg', fig.align = "center", fig.asp = 1, fig.width = 7}
...
```{r fig.align = "center", fig.asp = 1, fig.width = 7, fig.height = 5}
anim <- 
